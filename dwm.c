...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - (int)TEXTW(ctext) - barborderpx)
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	drw_rect(drw, x - 2 * sp - barborderpx, barborderpx, w, bh, 1, 1);
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x - 2 * sp - barborderpx, barborderpx, w, bh, 0, text, 0);
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					drw_rect(drw, rx + x - 2 * sp - barborderpx, ry, rw, rh, 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = m->ww - drawstatusbar(m, th, stext);
	}
	...
	if ((w = mw - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, x, y, w - 2 * sp + barborderpx, th, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs, y + boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_rect(drw, x, y, w - 2 * sp + barborderpx, th, 1, 1);
		}
	}
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + extrabarheight + barborderpx * 2;
	sp = sidepad;
...
warp(const Client *c)
{
	...
	if (!getrootptr(&x, &y) ||
		(x > c->x - c->bw &&
		 ...
		 x < c->x + c->w + c->bw*2 &&
		 y < c->y + c->h + c->bw*2) ||
		(y > c->mon->by && y < c->mon->by + bh + vertpad) ||
		(c->mon->topbar && !y))
		return;
